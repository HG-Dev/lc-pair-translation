import json
from typing import Iterator, Optional, Any, LiteralString
...
class ConfigOld:
    ...
    @property
    def chunk_min_length(self) -> PositiveInt:
        """ Minimum length of a chunk """
        return self._config_dict[CHUNK_SETTINGS_H]['int_MinLength']
    
    @property
    def chunk_max_length(self) -> PositiveInt:
        """ Maximum length of a chunk """
        return self._config_dict[CHUNK_SETTINGS_H]['int_MaxLength']

    @property
    def current_chunk_divider_patterns(self) -> list[Pattern]:
        return [pattern for (key, pattern) in self._config_dict[CHUNK_PATTERNS_H].items() if not key.startswith('#')]
